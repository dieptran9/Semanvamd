...
# CM Results
cm_results <- read.xlsx("D:/Data/R/Semanvamd/semanvamd-results-cm-sensitivity-analysis-1-series2.xlsx",1) #SemaNVAMD
...
cm_results <- cm_results %>% 
  filter(target_cohort_id==201) # for Specific NVAMD
...
# Choose facet_wrap to order the subtitle as needed: (1) alphabet order (2) customize order
                  ...
                  scale_x_log10(limits = c(0.05,20), breaks = c(0.05, 0.1, 0.2, 0.5, 1, 2, 5, 10), labels = c("0.05", "0.1","0.2", "0.5", "1", "2", "5", "10")) +
                  ...
                  theme(panel.grid.minor = element_blank(),
                        ...
                        # plot.margin = unit(c(1, 2, 1, 1), "lines")) + # without the legend for presentation purpose
                        plot.margin = unit(c(1, 2, 0, 1), "lines")) + # with the legend for ARVO paper
...
# Choose facet_wrap to order the subtitle as needed: (1) alphabet order (2) customize order
                  ...
                  scale_x_log10(limits = c(0.01,20), breaks = c(0.05, 0.2, 0.5, 1, 2, 5, 10), labels = c("0.05","0.2", "0.5", "1", "2", "5", "10")) +  #main model 300
                  scale_color_identity() +
...
height_in <- 8.5 # DN: <- adjust for each images
file_name <- "Sensitive_1_series2" # DN: <- adjust for file name
CM_combined_grobs |>
...
df_text <- data.frame(
  ...
  label = "<b>Abbreviations: </b> NVAMD = neovascular age-related macular degeneration, CI = confidence interval<br>
             ...
             Clinformatics = Optum Clinformatics Data Mart - Extended Data Mart – Socioeconomic Status<br>
             MDCR = Merative MarketScan Medicare Supplemental and Coordination of Benefits Database<br>
             Optum EHR = Optum de-identified Electronic Health Record data set<br>
...
height_in <- 10 # DN: <- adjust for each images
file_name <- "Sensitive_1_series2_ARVO" # DN: <- adjust for file name
